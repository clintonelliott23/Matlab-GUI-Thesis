function The_Solar_Solution_GUI()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%% Script for Solar Solution GUI %%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Initial Clearing  (TAB JUMP)
    % Clear and close all operations and variables
        clear all
        close all
        clc
...
%% Create a Figure for GUI
    % Creates the maine figure for the GUI which all handles use
        hTabFig = figure(...
            'Units', 'pixels',...
            'Toolbar', 'none',...
...
%% Build the Tabs
    %Creates tabs for the GUI in a for-loop
...
%% Define the callbacks for the Tab Buttons, Clock, Prefill and Reset Button
   ...
   %{ 
    NOTE: This button is used for testing the program and fault finding slash error checking   
    Create a Prefill button
    %}  
        prefill_button = uicontrol('Units', 'normalized', 'Position',[0.95 0.05 0.05 0.05], 'Style', 'pushbutton',...
            'String', 'E.X', 'Visible', 'On','Callback', @prefill,'Parent', TabHandles{1,1},...
            'Backgroundcolor',Grey, 'Foregroundcolor', 'black', 'FontSize', 15);          
                    
          
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        %%   Tab 1 Content: PROMPTING TAB %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
...
%% Gas Connected
	% Create the gas related questions
    ...
    function gas_click(hObject, eventdata)
                % Set previous questions off and next on with progress
...
%% Pool Connected
	% Create the pool related questions
...
%% Percentage Usage 
    ...
    % Create function selection click     
    function percentage_next(hObject, eventdata)
        % Create a next button once an option has been selected
...
%% Roof Orientation
    ...
    % Load Compass Image
        ...
        compass_image = uicontrol('style','pushbutton','units','normalized','position',[0.333 0.13 0.33 0.55],...
                'cdata',I2, 'Visible', 'Off','Parent', TabHandles{prompt_page,1});
    ...
    % Find which popupmenu was selected and update the variable display box
        ...
        % If a selection other than North is chosen display an error
...
%% Post Code
    ...
    % Dropdown list for post codes
        state_codes = [4814 4825 0800 6000 3000 7000 2000 4000];
        
    % List of names of post codes
    ...
    % Create table to display postcode options
    ...
    % Create function for click   
        function state_check_next(hObject, eventdata)    
           % Computes the state code
                % Displays the post code
                ...
                % Checks the post code against all options for errors
                for i = 1:1:length(state_codes)       
                       if state_input == state_codes(i)
                         ...
                         % Displays error for no gas data                                                
...
%% Number of People
    ...
    % Create function for click  
        function people_click(hObject, eventdata)
            ...
            % Run the main function to calculate inputs
            ...
            % Display the input tab, update progress and turn on the enter prompt                
...
%% Create the UI Handles for the labels and Values on Production Page
    ...
    % This is the main code used in program
            function PSH_and_KW_Calc(solar_size_input,performance_input, roof_tilt_input,state_input...
                ...
                % Calculates how much is imported from the grid
                ...
                % Updates the values to the screen for the user
                ...
                % Runs the functions for cost and plots the values     
                    production_graph() 
                     cost_analysis(daily_savings)
                     finance_graph()

                % Protects the pie graph against non-positive data
                   if (daily_storage <= 0)
                ...
                % Code for the Pie Chart creations
               ...
               % This code is to name the pie chart and name the percentages                             
                ...
                % This was the larger pie chart with the same values but on
                % another tab
	...
	% Function for finding the linear change between tilt angle and NASA
	% PSH
            function [PSH_avg] =  tilt_calculator (roof_tilt_input,month)
                % Create variables for rows and columns
                    psh_row_index = 0;
                    month = month + 1;
                % Switch statements determines the post code
                        switch state_input
                            case 4814                        
                            case 4825
                                psh_row_index =  5;
                 ...
                 % Data creates the min and max values        
                ...
                % If statements index the data for analysis
                ...
                % The PSH is calculated using the min and max tilts and a
                % linear equation
        ...
        % Function for plotting the production
            function production_graph()
                ...
                % Create a cell array for the months to be plotted
                    month_name = ({'Jan';'Feb';'Mar';'Apr';'May';'Jun';'Jul';'Aug';'Sep';'Oct';'Nov';'Dec';'Average'});         
                % Create a looop to plot the production agasint the months
                    for i = 1:1:13
                ...
                % Create graph larger and on another tab for report
                    % Create another axis
      ...
      % Main fucntion for indexing       
            function [kwhr_avg_found] = average_kwhr_finder(state_input,number_people_input,gas_mains_input, pool_input)
                ...
                % Switchstatement from state input       
                 ...
                 % Switch statement for number of occupants
                 ...
                 % Switch statement for pool connection                  
                   switch pool_input                     
                    case 1
                        row = row +2;
                   end

                 % Switch statement for gas mains               
                   switch gas_mains_input
                    case 1
                        row = row +1;
                   end

                 % From the stored data the average kW is indexed                
        ...
        function finance_graph()
            ...
            % interest
                    for i = 1:1:20 
                        ...
                        % Inflation for normale rate and solar
                       yearly_cost_plot = yearly_cost_plot *(1+inflation_rate);
                       yearly_cost_witsol_plot = yearly_cost_witsol_plot * (1+inflation_rate);
                    end 

            % This is for if a negative value (i.e savings) are found then
            % the bar chart cannot plot and this is swaps the axis and type
            % of bar graph
            ...
            % This was another graph used on another tab to use in report
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Create the UI Handles for the labels and Values on Display Page
    % Create tab reference for parent handles
        y_disp_offset = 0.025;
...
%%   Save the TabHandles in guidata
    % Used to save the tabs
        guidata(hTabFig,TabHandles);

%%   Make Tab 1 active
    % Makes a selected tab active after the initialisation
        TabSellectCallback(0,0,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        %%   Background Images (TAB JUMP)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Create the background pictures for each tab
% This would not work in a loop for some reason so it was duplicated here
% in the code
	%%%%%%%%%%%%%%%%%1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This sections computes all the costing for the GUI
    % Create persistent Variables to be used
    ...
    % Main function for cost analysis
        function cost_analysis(daily_savings)
            ...
            % Investment_cost and IRR Calculation
    ...
    % Used to update the production values
        function Update_Values_prod(kwhr_avg_found,kw_produced_daily,daily_storage,tariff_rate_normal_found,daily_exported,daily_imported,daily_savings)
            % Sends to the economic tab the calculate values
                  ...
                  set(disp_used_value,'string', num2str(kwhr_avg_found));                                           disp(source_energy)             
    ...
    % Used to update the cost values
    ...
    % Used to indicate to the user the time
        function clock_disp()
        %  Retrives the time from the computer      
                time_all = fix(clock);
                hr = time_all(1,4);
                min = time_all(1,5);

        %  Creates the axis for plotting
    ...
    % Used to indicate to the user what percentage is completed
        function progress_bar(progress_step)
            ...
            % Displays the text percentage numerically
    ...
    % Used in the rest of the program 
        function reset(hObject, eventdata) 
            % Resets all the inputs and displays
                 ...
                 % pauses, closes, displays error and restarts the GUI            
                    pause(0.75)
                    close(gcbf)%            
                    errordlg('Error Restarting, Reboot Initiated','Setup Error')            
                    The_Solar_Solution_GUI()
                    set(prefill_button,'Visible','On')
        end    
    
    
    % Used in Prefill function to clear all uipanels
        function all_visible_off(hObject, eventdata)
            % Turns off all the button when prefill is selected
                    set(gas_mains_question,'Visible','Off') 
                    set(button_yes_gas_mains,'Visible','Off') 
                    set(button_no_gas_mains,'Visible','Off') 
                    set(pool_question,'Visible','Off') 
                    ...
                    set(text_orientation_question,'Visible','Off')  
                    set(compass_image,'Visible','Off')                 
                    set(radio_north_button,'Visible','Off');                
                    set(radio_north_west_button,'Visible','Off')    
                    set(radio_east_button,'Visible','Off');                 
                    set(radio_north_east_button,'Visible','Off')  
                    set(radio_south_button,'Visible','Off');                
                    set(radio_south_east_button,'Visible','Off')  
                    set(radio_south_west_button,'Visible','Off') 
                    set(state_display_button,'Visible','Off')         
                    set(state_next_button,'Visible','Off') 
                    set(text_state_question,'Visible','Off')  
                    set(postcode_edit,'Visible','Off')      
    ...
    % Used in prefilling the GUI with data instead of the prompt process
        function prefill(hObject, eventdata)
            % Progress bar and turn off uihandles
                progress_bar(number_of_inputs);            
                all_visible_off()      
                ...
                % Set values to be prefiled here...
                    gas_mains_input = 0;               
                    pool_input = 0;                     
                    percentage_input = 30;
                    ...
                    solar_installed = 1;                    
                    solar_size_input = 5;              
                    cost_solar_input = 7000;            
                    battery_installed = 1;
                    battery_size_input =  10;         
                    cost_battery_input =  9700;                           
                    orientation_input = 1;              
                    bill_input = 0;                    
                ...
                % Set values on the GUI for display
                    set(pool_value, 'String', 'No')                   
                    set(solar_size_value, 'String', num2str(solar_size_input))                   
                    set(solar_cost_value, 'String', num2str(cost_solar_input))                 
                    set(battery_size_value, 'String', num2str(battery_size_input) )                
                    set(battery_cost_value, 'String', num2str(cost_battery_input) )                   
                    set(orientation_value, 'String', 'North')                
                    set(bill_value, 'String', num2str(bill_input) )                                    
                    set(occupants_value, 'String', num2str(number_people_input))
                    set(gas_main_value, 'String', 'No')              
                    ...
                    set(enter_gui_button,'Visible','On')
                    close(loading_bar)
                % Calls the main function for calculations
                    PSH_and_KW_Calc(solar_size_input,performance_input, roof_tilt_input,state_input...
                    ,number_people_input,gas_mains_input, pool_input, battery_size_input, solar_installed, battery_installed)                     

                % Runs through each tab giving the user an overview of the 
                % program
...
%%   Callback for Tab Selection (TAB JUMP)
    function TabSellectCallback(~,~,SelectedTab)
        %{ 
            All tab selection pushbuttons are greyed out and uipanels are set to visible off,
            then the selected panel is made visible and it's selection pushbutton is highlighted       
        %}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% End of Script for Solar Solution GUI Display %%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%