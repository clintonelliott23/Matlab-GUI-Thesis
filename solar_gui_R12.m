...
persistent yearly_savings_adj_infla
    function finance_graph()
        ...
        for i = 1:1:20                     
                       bar_data_finance(1,i) = yearly_cost_plot;
                       bar_data_finance(2,i) = yearly_cost_witsol_plot;
        ...
        xlabel('Years','FontSize', 16);                      ylabel('Cost $AUD','FontSize', 16);          
         ...
         legend({'Solar/Batery','Normal'},'Location','northwest');
        set(gca, ...
          'Box'         , 'off'     , ...
          'TickDir'     , 'out'     , ...
          'TickLength'  , [.01 .01] , ...
...
%             salvage_cost = solar_size_input*0.21*1000;
            ...
            LCC = cost_solar_input*1000 + maintenance_cost
            ALCC = LCC/pa;
...
function Update_Values_cost(electricity_cost_ALCC,electricity_cost_ANNPMT,daily_savings,payback_period,ROI)
    ...
    function clock_disp()
        ...
        hr = time_all(1,4)
        min = time_all(1,5)    
        check = uicontrol('Style','text','Units', 'normalized','Position',[0.0 0.0 0.04 0.03],... 
...
function progress_bar(progress_step)
    disp(round(progress_step*100/number_of_inputs,1))
        ...
        uicontrol('Style', 'text','Units', 'normalized','Position', [0.8 0.06 0.15 0.04],'Parent', TabHandles{1,1},... 
            'string',sprintf('Progress: %d%%',round(number_of_inputs*100/number_of_inputs)),'BackgroundColor','white',... 
            'HorizontalAlignment','left','FontName','arial','FontWeight','bold', ... 
            'FontSize', 12);         
end
